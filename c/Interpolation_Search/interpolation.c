...
void  interpolation_Search_recursive(int* arr,int low,int high,int element){
    if(low<=high){
        ...
        if(arr[mid]==element){
            ...
            return interpolation_Search_recursive(arr,mid+1,high,element);
        }else{
            return interpolation_Search_recursive(arr,low,mid-1,element);
        }
    }
...
void interpolation_Search_iterative(int* arr,int size,int element){
    int low=0;
    int high=size-1;
    int mid;
    while(low<=high){
...
int main(){
    ...
    scanf("%d",&choice);
    if(choice==1){
        interpolation_Search_recursive(arr,0,size-1,element);
        break;
    }
    else if(choice==2){
        interpolation_Search_iterative(arr,size,element);
        break;
    }
    else{
